...
function get_data($auth_token, $arguments)
{
    ...
    */
    ...
    $q = " select value,from_unixtime(utimestamp, '%Y-%m-%dT%H:%i:%s') as timestamp from stats_data where id_item=" . $config["dbh"]->prepare($arguments->oid, "number") . " and utimestamp > (unix_timestamp(now()) - 172800) order by utimestamp desc";
